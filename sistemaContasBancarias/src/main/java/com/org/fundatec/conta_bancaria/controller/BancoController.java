...
public class BancoController {
    ...
    public ResponseEntity<Banco> get(@PathVariable("banco") Integer numId)throws RegistroNaoEncontradoException {
        Banco retorno = bancoService.busca(numId);
    ...
    public ResponseEntity<List<Banco>> get(@RequestParam("nome") String nome) {
        return ResponseEntity.status(HttpStatus.OK).body(bancoService.buscarNomeAprox(nome));
    }

    @GetMapping(value = { "{banco}" }, produces = {MediaType.APPLICATION_JSON_VALUE})
    public ResponseEntity<Banco> buscaPorCodigo(@PathVariable("banco") Integer codigo)throws RegistroNaoEncontradoException {
    ...
    @PutMapping(value = { "{banco}" }, consumes = MediaType.APPLICATION_JSON_VALUE,
            produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<Banco> editar(@PathVariable("banco") Integer numId, @RequestBody Banco banco) throws RegistroNaoEncontradoException{
        return ResponseEntity.status(HttpStatus.OK).body(bancoService.editar(numId, banco));
    }

    @DeleteMapping(value = { "{banco}" },
            consumes = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<Banco> remover(@PathVariable("banco") Integer numId) throws RegistroNaoEncontradoException{
        bancoService.remover(numId);
