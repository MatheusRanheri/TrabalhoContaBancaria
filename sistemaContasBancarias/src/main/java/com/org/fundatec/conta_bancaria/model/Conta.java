...
public class Conta {
    ...
    @NotBlank(message = "O atributo número é obrigatorio")
    private Integer numero;

    private Double saldo;

    @NotBlank(message = "A agencia é um atributo obrigatorio")
    @ManyToOne(fetch = FetchType.LAZY, cascade = CascadeType.ALL)
    @JoinColumn(name = "id_agencia", referencedColumnName = "id")
    private Agencia agencia;

    @NotBlank(message = "O cliente é um atributo obrigatório")
    @ManyToOne (fetch = FetchType.LAZY, cascade = CascadeType.ALL)
