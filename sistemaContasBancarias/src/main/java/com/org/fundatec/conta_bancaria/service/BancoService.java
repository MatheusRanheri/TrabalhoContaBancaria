...
public class BancoService {
    ...
    public List<Banco> buscarNomeAprox(String nome){
        List<Banco> bancos = bancoRepository.findBancoByNomeContains(nome);
        if(bancos.isEmpty()){
            throw new RegistroNaoEncontradoException("Nenhum banco encontrado com esse nome");
        }
    ...
    public List<Banco> buscarTodos(){
        List<Banco> bancos = StreamSupport.stream(bancoRepository.findAll().spliterator(), false).toList();
        if(bancos.isEmpty()){
            throw new RegistroNaoEncontradoException("Nenhum banco encontrado");
        }
